import { MathOperator, Method, Payload } from '@joshdb/core';
import { JSONProvider, JSONProviderError } from '../../src';

describe('JSONProvider', () => {
	describe('is a class', () => {
		test('GIVEN typeof JSONProvider THEN returns function', () => {
			expect(typeof JSONProvider).toBe('function');
		});

		test('GIVEN typeof ...prototype THEN returns object', () => {
			expect(typeof JSONProvider.prototype).toBe('object');
		});
	});

	describe('can manipulate provider data', () => {
		const provider = new JSONProvider({ dataDirectoryName: '.tests' });
		...
		describe('with autoKey method', () => {
			test('GIVEN ... THEN returns payload w/ generated key as data AND increments autoKeyCount', async () => {
				...
				const index = await provider['handler'].index.fetch();

				expect(index.autoKeyCount).toBe(1);
			});
		});
