import { JoshProvider, JoshProviderError, MathOperator, Method, Payload } from '@joshdb/core';
import type { Awaitable } from '@sapphire/utilities';

export function runProviderTest<
  Provider extends JoshProvider.Constructor,
  ...
  describe(Provider.prototype.constructor.name, () => {
    ...
    describe('can manipulate provider data', () => {
      ...
      describe('with map method', () => {
        describe('hook type', () => {
          test('GIVEN provider w/o data THEN returns payload w/o data from map', async () => {
            const payload = await provider.map({ method: Method.Map, type: Payload.Type.Hook, hook: (value: any) => value, data: [] });
      ...
      describe('with update method', () => {
        test('GIVEN provider w/o data THEN returns payload w/o data', async () => {
          const payload = await provider.update({ method: Method.Update, key: 'test:update', path: [], hook: (value: any) => value });
        ...
        test('GIVEN provider w/ data at path THEN returns payload w/ data AND updates value at path', async () => {
          ...
          const payload = await provider.update({ method: Method.Update, key: 'test:update', path: ['path'], hook: (value: any) => value });
