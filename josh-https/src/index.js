...
class JoshProvider {
  ...
  constructor(options) {
    ...
    if (!options.url) {
      throw new Error('Must provide options.url');
    }
   ...
   */
  async init() {
    const res = await this.fetch('/check');
    console.log(res);
    if (res.message === 'ok') {
  ...
  async get(key, path) {
    ...
    const res = await this.fetch(`/key/${key}`);
    if (res.key !== key) {
      throw new Error('Key does not match. Wat?');
    }
    const value = this.parseData(res.value);
    if (path) {
   ...
   */
  async getAll() {
    const all = await this.fetch('/keys');
    console.log(all)
    return this.parseKeyValObj(all);
  }
   ...
   */
  async getMany(keys) {
    const res = await this.fetch('/keys/many', {
      method: 'POST',
      body: { keys },
      headers: {
    ...
    return this.parseKeyValObj(res);
  }
  ...
  async random(count = 1) {
    const res = await this.fetch(`/random/values/${count}`);
    return this.parseKeyValObj(res);
  }
  ...
  async randomKey(count = 1) {
    const res = await this.fetch(`/random/keys/${count}`);
    return this.parseKeyValObj(res);
  }
   ...
   */
  async has(key, path) {
    const res = await this.fetch(`/has/${key}`);
    return res;
  }
   ...
   */
  async values() {
    const values = await this.fetch('/list/values');
    return values.map(this.parseData);
  }
   ...
   */
  async count() {
    const { count } = await this.fetch('/list/size');
    return count;
  }
  ...
  async set(key, path, val) {
    ...
    const stringified = serialize(val);
    const res = await this.fetch(`/key/${key}`, {
      method: 'POST',
      body: { value: stringified },
      headers: {
        'Content-Type': 'application/json',
      },
    });
